...
@Component({
  selector: 'ispl-debit-note-item-details-view',
  templateUrl: 'debit-note-item-details-view.component.html',
  styleUrls: ['debit-note-item-details-view.component.css']
})
export class DebitNoteItemDetailsViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    console.log("OnInit debit note item ViewComponent Class");
  ...
  private getRouteParam() {
    ...
    routeSubscription = this.activatedroute.params.subscribe(params => {
      this.creditNoteNumber = params.debinoteeno ? params.debinoteeno : '';
      this.statusQuery = params.debinoteeno ? params.debinoteeno : '';
    });
  ...
  private invoiceItemDetViewWSCall() {
    ...
    this.purchaseInvoiceItemViewDataService.getDebitNoteItemDetViewWithFacet(this.sortSelection).
      subscribe(res => {
        ...
        if (res.msgType === "Info") {
          ...
          this.creditNoteItemViewDetails.processStartDateTime = res.processStartDateTime;
        }
        ...
        err => {
          ...
          this.creditNoteItemViewDetails.processStartDateTime = err.processStartDateTime;
          this.busySpinner.gridBusy = false;
          this.updateBusySpinner();
        });
  }//end of method
  ...
  public onClickBreadcrumbLink(val) {
    ...
    if(val==="Invoice") {
      ...
      this.router.navigate([ROUTE_PATHS.RouteDebitNoteDetView, this.invoiceNo, postatus]);
    }
  }//end of method
