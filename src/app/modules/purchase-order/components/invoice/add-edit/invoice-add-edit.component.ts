...
@Component({
  selector: 'ispl-invoice-add-edit-form',
...
export class InvoiceAddEditComponent implements OnInit {
  public title: string = "Add Invoice";
  ...
  public itemQtyCgstIgstError: boolean = false;
  ...
  public invoiceDateErr: string = "";
  public invoiceDateErrFlag: boolean = false;
  ...
  constructor(
    ...
    console.log("this.localStorageService.user:::::", this.localStorageService.user.userId);
    this.initform();
  ...
  ngOnInit(): void {
    ...
    console.log(" itemQtyCgstIgstError ", this.itemQtyCgstIgstError);
  ...
  */
  initform() {
    this.invoiceAddEditFormGroup = new FormGroup({
      ...
      division: new FormControl('DI', Validators.required)
    });
  ...
  private getPONosWs(code: string, vendorGstInNo: string, division: string) {
    ...
    this.invoiceAddEditDataService.getPONosVal(code, vendorGstInNo, division).
      subscribe(res => {
        console.log(" po nos========>>>>>>>>>>", res);
        this.poNoDropDownVal = res.details;
        this.busySpinner.selectedBusy = false;
        this.updateBusySpinner();
        for (let poNo of this.poNoDropDownVal) {
          // if (this.complaintTypeIdForModify == "" || this.complaintTypeIdForModify == undefined) {
          if (poNo.Key == "") {
            this.invoiceAddEditFormGroup.controls["poNo"].setValue(poNo.Key);
            break;
          }//end if
          // }//end of if
        }//end for
  ...
  private onFileUploadWS(paramValue: string) {
    ...
    this.updateBusySpinner();
    this.invoiceAddEditDataService.fileUpload(formDataObj, paramValue).
      subscribe(res => {
  ...
  private fileDeleteWs(paramValue: string, fileName: string) {
    ...
    this.invoiceAddEditDataService.fileDelete(fileDetails, paramValue).
      subscribe(res => {
  ...
  private getSelectedDescValWs() {
    this.invoiceAddEditDataService.getSelectedDescVal().
      subscribe(res => {
        ...
        if (res.msgType === "Info") {
          ...
          console.log(" this.descDropDownVal ===========", this.descDropDownVal);
        }//end of if
      },
        err => {
          console.log(err);
  ...
  private getGstPercentage() {
    ...
    for (let chkItm of this.checkedItemArr) {
      if (chkItm.taxRate != 0) {
        ...
        this.onKeyupToGenerateCgstIgstSgst(chkItm.taxRate, chkItm);
      }//end of if
    }//end of for
  }//end of the method
  ...
  private getItemDetailsWS(poNo: string) {
    this.busySpinner.selectedBusy = true;
    this.updateBusySpinner();
    this.invoiceAddEditDataService.getItemDetVal(this.code, poNo, this.vendorGstInNo).
      subscribe(res => {
        // this.itemDetails = res.details;
        this.itemsHeader = res.header;
        // let itmListFormControl: any = new FormControl(false);
        // this.itemListFormGroup = new FormGroup(itmListFormControl);
  ...
  private getVendorDetValWS(vendorCode: string) {
    ...
    this.busySpinner.selectedBusy = true;
    this.updateBusySpinner();
    this.invoiceAddEditDataService.getVendorDetVal('', vendorCode, this.division).
      subscribe(res => {
        ...
        if (res.msgType === 'Info') {
          ...
          // let resDet = res.details[0];
            ...
            for (let vendorDet of res.details) {
              vendorCode = vendorDet.vendorCode;
              this.itemDetailsService.vendorCode = vendorDet.vendorCode;
              this.itemDetailsService.vendorName = vendorDet.vendorName;
              this.itemDetailsService.vendorGstin = vendorDet.vendorGstin;
              this.itemDetailsService.vendorPanNo = vendorDet.vendorPanCardNo;
              this.itemDetailsService.testVar = "data stored";
            }
            console.log("complaintRefId for modify : ", vendorCode);

            this.router.navigate([ROUTE_PATHS.RouteInvoiceAddWithVendorCode, vendorCode]);
  ...
  private onOpenModal(invoiceTransactionNo: string) {
    ...
    modalRef.componentInstance.modalMessage =
      ...
      "Invoice Scan ID " + invoiceTransactionNo + " created successfully.";
  }
  ...
  public onVendorCodeSearch(vendorCode: string) {
    ...
    if (vendorCode) {
      ...
      this.router.navigate([ROUTE_PATHS.RouteVendorCodeSearch, "invoice"]);
    }
  ...
  onVendorCodeKeyUp(vendorCode: string) {
    if (!vendorCode) {
      this.vendorCodeError = false;
  ...
  private complaintQtyCgstSgstErrorCorrection() {
    for (let checkedItm of this.checkedItemArr) {
      ...
      if (checkedItm.uiTaxtRateErrFlag || checkedItm.uiItemNameErrFlag || checkedItm.uiItemValueErrFlag || ((this.poType === 'ZRDS' || this.poType === 'ZCDS') && this.poUnit === 'JOB' && orderQtyNum === 1 && checkedItm.uiItemValueErrFlag)) {
        this.itemQtyCgstIgstError = true;
        break;
      } else if ((checkedItm.uiTaxtRateErrFlag === false || checkedItm.uiTaxtRateErrFlag === undefined) && (checkedItm.uiItemValueErrFlag === false || checkedItm.uiItemValueErrFlag === undefined) && (checkedItm.uiItemNameErrFlag === false || checkedItm.uiItemNameErrFlag === undefined) || ((this.poType === 'ZRDS' || this.poType === 'ZCDS') && this.poUnit === 'JOB' && orderQtyNum === 1 && (checkedItm.uiItemValueErrFlag === false || checkedItm.uiItemValueErrFlag === undefined))) {
        this.itemQtyCgstIgstError = false;
      }
  ...
  public invoiceAddSubmit() {
    ...
    console.log(" this.invoiceFilesArr[0].fileName; ", this.invoiceFilesArr[0].fileName);
    invoiceTranDet.invoiceFileName = this.invoiceFilesArr[0].fileName;
    invoiceTranDet.vendorCode = this.code;
    invoiceTranDet.poNumber = this.invoiceAddEditFormGroup.value.poNo;
    invoiceTranDet.invoiceNumber = this.invoiceAddEditFormGroup.value.invoiceNumber;
    invoiceTranDet.invoiceDate = this.invoiceAddEditFormGroup.value.invoiceDate;
    invoiceTranDet.invoiceAmount = this.totalInvoiceAmount;
    ...
    this.busySpinner.submitBusy = true;
    this.updateBusySpinner();
    this.invoiceAddEditDataService.transactionAddSubmit(invoiceTranDet, plantType).
      subscribe(res => {
  ...
  public onItemNameKeyup(itemNameParam: string, itemCodeParam: string, lineItemNoParam: string, itemNoParam: string) {
    ...
    }//end of else if potype === FO || poType === ZS
  ...
  public onDivisionChoose() {
    console.log(" division =======xxxx", this.division);
    this.division='DI'
  ...
  public onChangeInvoiceDate() {
    this.invoiceDateErrFlag = false;
    this.invoiceDateErr = "";
    //formatting the current date
    let date = new Date();
    this.currentDate = this.datePipe.transform(date, 'yyyy-MM-dd');
    if ((new Date(this.invoiceAddEditFormGroup.controls['invoiceDate'].value) > new Date(this.currentDate))) {
      console.log("fromDate Date error.")
      this.invoiceDateErrFlag = true;
      this.invoiceDateErr = "Invoice Date can\'t be greater than Today's Date!";
    }//end of if
  }//end method of onChangeInvoiceDate
