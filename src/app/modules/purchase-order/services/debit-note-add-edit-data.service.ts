...
@Injectable()
export class DebitNoteAddEditDataService {
  ...
  getItemDetVal(vendorCode: string, poNo: string, invoiceNo: string, vendorGstInNo: string){
    let actionUrl = AppUrlsConst.ITEM_DET_BY_PO_NO_URL+ "/" + vendorCode + "/" + poNo+ "/" + invoiceNo;
    if(vendorGstInNo){
  ...
  transactionAddSubmit(tranAddDet: any, plantType: string) {
    this.configService();
    this.actionUrl = AppUrlsConst.DEBIT_NOTE_ADD_SUBMIT_URL+ "/" + plantType;
